...
svr_q_dic = {"semantics" : {"entity": "PID", "baseentity" : "any", "role" : "plan_points1", "spatial" : "any", "temporal" : "any"}, "dataproperty": {"format": "inaoka_matrix"}}
...
while True:
    ...
    # Create an iris object
        ...
        cliq1 = cli_dic.copy()
        cliq1["semantics"]["role"] = "T_Camera_Plan1"
        
        
        port_data, addr_data = UDP_broadcast(json.dumps(cliq1, ensure_ascii=False))
        if json.loads(port_data)["msg"] == "ok":
			
            T_Camera_Plan1_bin = TCP_data_rcv(addr_data,port_data)
            print(T_Camera_Plan1_bin)
            T_Camera_Plan1_ls = json.loads(T_Camera_Plan1_bin)["T_Camera_Plans1"]
            T_Camera_Plan1_array = np.array(T_Camera_Plan1_ls)
            print(T_Camera_Plan1_array)        
        ...
        print(plan_dic)
        plan_length = plan_dic["length"]
        #カメラjson
        camera_json = get_data_ID(cls="ion.navi",R="camera.json", E="camera", BE="polaris", T="2023", S="TMDU/nkjlab",P=json.dumps({"format": "inaoka_json"}), start=1,end=(maxID+1))
        data_camera = get_data(camera_json[0]["ID"])
        camera_dic = json.loads(data_camera)
        
        camera_intrinsic = np.array(camera_dic["intrinsic"]) #行列
        plan_points1 = projection(T_Camera_Plan1_array, plan_length, camera_intrinsic)#行列,<class 'numpy.ndarray'>
        

        plan_points1_json = json.dumps({"plan_points1" : plan_points1.tolist()})
        client.send(plan_points1_json.encode())
        conn.close()
        break
