...
svr_q_dic = {"semantics" : {"entity": "PID", "baseentity" : "-", "role" : "T_Camera_Tool1", "spatial" : "any", "temporal" : "any"}, "dataproperty": {"format": "inaoka_matrix"}}
...
while True:
    ...
    while True:
        ...
        cliq1 = cli_dic.copy()
        cliq1["semantics"]["role"] = "T_BoneTracker_Tool"
        
        port_data, addr_data = UDP_broadcast(json.dumps(cliq1, ensure_ascii=False))
        if json.loads(port_data)["msg"] == "ok":
			
            T_BoneTracker_Tool_bin = TCP_data_rcv(addr_data,port_data)
            T_BoneTracker_Tool_ls = json.loads(T_BoneTracker_Tool_bin)["T_BoneTracker_Tool"]
            T_BoneTracker_Tool_array = np.array(T_BoneTracker_Tool_ls)        
        

        cliq2 = cli_dic.copy()
        cliq2["semantics"]["role"] = "T_Camera_CameraTracker"
        ...
        #欲しいプランの番号、結果から取得
        plan1_json = get_data_ID(cls="ion.navi", R="surgical_result",E= "PID001",BE="-",T="2023/05/02",S="TMDU/nkjlab", P=json.dumps({"format": "inaoka_json"}), start=1,end=(maxID+1))
        data_plan1 = get_data(plan1_json[0]["ID"])
        
        plan1_dic = json.loads(data_plan1)
        plan1_num = plan1_dic["PlanNumber"][0]
        ...
        T_Camera_Tool1 = T_Camera_CameraTracker_array.dot(np.linalg.inv(T_CameraTracker1)).dot(T_BoneTracker1).dot(T_BoneTracker_Tool_array)
        print(T_Camera_Tool1)
        T_Camera_Tool1_json = json.dumps({"T_Camera_Tool1" : T_Camera_Tool1.tolist()})

        client.send(T_Camera_Tool1_json.encode())
        conn.close()
        break
