...
svr_q_dic = {"semantics" : {"entity": "PID", "baseentity" : "-", "role" : "T_Camera_Plan1", "spatial" : "any", "temporal" : "any"}, "dataproperty": {"format": "inaoka_matrix"}}
...
while True:
    ...
    while True:
        ...
        cliq1 = cli_dic.copy()
        cliq1["semantics"]["role"] = "T_Camera_CameraTracker"
        
        
        port_data, addr_data = UDP_broadcast(json.dumps(cliq1, ensure_ascii=False))
        if json.loads(port_data)["msg"] == "ok":
			
            T_Camera_CameraTracker_bin = TCP_data_rcv(addr_data,port_data)
            print(T_Camera_CameraTracker_bin)
            T_Camera_CameraTracker_ls = json.loads(T_Camera_CameraTracker_bin)["T_Camera_CameraTracker"]
            T_Camera_CameraTracker_array = np.array(T_Camera_CameraTracker_ls)   

        # insert = irispy.classMethodValue("ion.navi","InsertNewpy", "surgical_result", "PID001","-",json.dumps(data),"2023/05/02","TMDU/nkjlab", json.dumps({"format": "inaoka_json"}))
        result_json = get_data_ID(cls="ion.navi", R="surgical_result",E= "PID001",BE="-",T="2023/05/02",S="TMDU/nkjlab", P=json.dumps({"format": "inaoka_json"}), start=1,end=(maxID+1))
        data_result = get_data(result_json[0]["ID"])
        
        result_dic = json.loads(data_result)
        Plan1_num = result_dic["PlanNumber"][0]

        #トラッカー情報を取得、DBがjson、プランのトラッカーは不変値
        #data_name = cli_dic["semantics"]["entity"] + "_tracker.json"
        data_name = str(Plan1_num)+"_tracker_matrix"
        #tracker_json = get_IRISobj_dic(cls="navi", R=data_name, E="tracker", BE="polaris", T="2023/05/02", S="TMDU/nkjlab",P=json.dumps({"format": "inaoka_json"}))
        tracker_json = get_data_ID(cls="ion.navi", R=data_name,E= "tracker",BE="polaris",T="2023/05/02",S="TMDU/nkjlab", P=json.dumps({"format": "inaoka_json"}), start=1,end=(maxID+1))
        data_tracker = get_data(tracker_json[0]["ID"])
        
        tracker_dic = json.loads(data_tracker)

        #カメラトラッカー
        T_CameraTracker1 = np.array(tracker_dic["CameraTracker"])
        #ボーントラッカー
        T_BoneTracker1 = np.array(tracker_dic["BoneTracker"])

        #プランの行列を取得、DBがjson
        # plan_insert = insert(Role="plan-20230502-newdataset.json", Entity="plan", BaseEntity="polaris",DataBody=json.dumps(data), Temporal="2023/05/02",Spatical="TMDU/nkjlab", Property=json.dumps({"format": "inaoka_json"}))

        plan_json = get_data_ID(cls="ion.navi", R="plan-20230502-newdataset.json",E= "plan",BE="polaris",T="2023/05/02",S="TMDU/nkjlab", P=json.dumps({"format": "inaoka_json"}), start=1,end=(maxID+1))
        data_plan = get_data(plan_json[0]["ID"])
        plan_dic = json.loads(data_plan)

        T_BoneTracker_Plan = np.array(plan_dic["matrix"])


        T_Camera_Plan1 = T_Camera_CameraTracker_array.dot(np.linalg.inv(T_CameraTracker1)).dot(T_BoneTracker1).dot(T_BoneTracker_Plan) #行列,<class 'numpy.ndarray'>

        T_Camera_Plan1_json = json.dumps({"T_Camera_Plans1" :  T_Camera_Plan1.tolist()})
        client.send(T_Camera_Plan1_json.encode())
        conn.close()
        break
