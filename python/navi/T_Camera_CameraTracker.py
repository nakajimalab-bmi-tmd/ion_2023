...
svr_q_dic = {"semantics" : {"entity": "PID", "baseentity" : "any", "role" : "T_Camera_CameraTracker", "spatial" : "any", "temporal" : "any"}, "dataproperty": {"format": "inaoka_matrix"}}
...
while True:
    ...
    while True:
        ...
        rotation = np.array(camera_dic['rotation'])
        camera_translation = np.array(camera_dic['translation'])
        T_CameraTracker_Camera = np.eye(4)
        T_CameraTracker_Camera[0:3, 0:3] = rotation
        T_CameraTracker_Camera[0:3, 3] = camera_translation
        T_Camera_CameraTracker = np.linalg.inv(T_CameraTracker_Camera)

        T_Camera_CameraTracker_json = json.dumps({"T_Camera_CameraTracker" : T_Camera_CameraTracker.tolist()})
        
        print(T_Camera_CameraTracker_json)
        client.send(T_Camera_CameraTracker_json.encode())
        conn.close()
        break
