...
svr_q_dic = {"semantics" : {"entity": "PID", "baseentity" : "-", "role" : "tool_points1", "spatial" : "any", "temporal" : "any"}, "dataproperty": {"format": "inaoka_matrix"}}
...
while True:
    ...
    while True:
        ...
        cliq1 = cli_dic.copy()
        cliq1["semantics"]["role"] = "T_Camera_Tool1"
        port_data, addr_data = UDP_broadcast(json.dumps(cliq1, ensure_ascii=False))
        if json.loads(port_data)["msg"] == "ok":
			
            T_Camera_Tool1_bin = TCP_data_rcv(addr_data,port_data)
            T_Camera_Tool1_ls = json.loads(T_Camera_Tool1_bin)["T_Camera_Tool1"]
            T_Camera_Tool1_array = np.array(T_Camera_Tool1_ls)        
        ...
        tool_points1 = projection(T_Camera_Tool1_array, plan_length, camera_intrinsic)#行列,<class 'numpy.ndarray'>
        
        print(tool_points1)
        tool_points1_json = json.dumps({"tool_points1" : tool_points1.tolist()})
        client.send(tool_points1_json.encode())
        conn.close()
        break
