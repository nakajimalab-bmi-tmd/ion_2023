...
svr_q_dic = {"semantics" : {"entity": "PID", "baseentity" : "-", "role" : "T_Camera_Tool2", "spatial" : "any", "temporal" : "any"}, "dataproperty": {"format": "inaoka_matrix"}}
...
while True:
    ...
    while True:
        ...
        #欲しいプランの番号、結果から取得
        plan2_json = get_data_ID(cls="ion.navi", R="surgical_result",E= "PID001",BE="-",T="2023/05/02",S="TMDU/nkjlab", P=json.dumps({"format": "inaoka_json"}), start=1,end=(maxID+1))
        ...
        T_Camera_Tool2 = T_Camera_CameraTracker_array.dot(np.linalg.inv(T_CameraTracker2)).dot(T_BoneTracker2).dot(T_BoneTracker_Tool_array)

        T_Camera_Tool2_json = json.dumps({"T_Camera_Tool2" : T_Camera_Tool2.tolist()})

        client.send(T_Camera_Tool2_json.encode())
        conn.close()
        break
