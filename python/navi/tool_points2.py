...
svr_q_dic = {"semantics" : {"entity": "PID", "baseentity" : "any", "role" : "tool_points2", "spatial" : "any", "temporal" : "any"}, "dataproperty": {"format": "inaoka_matrix"}}
...
while True:
    ...
    while True:
        ...
        cliq1 = cli_dic.copy()
        cliq1["semantics"]["role"] = "T_Camera_Tool2"
        port_data, addr_data = UDP_broadcast(json.dumps(cliq1, ensure_ascii=False))
        if json.loads(port_data)["msg"] == "ok":
			
            T_Camera_Tool2_bin = TCP_data_rcv(addr_data,port_data)
            T_Camera_Tool2_ls = json.loads(T_Camera_Tool2_bin)["T_Camera_Tool2"]
            T_Camera_Tool2_array = np.array(T_Camera_Tool2_ls)        
        ...
        plan_set_dic = json.loads(data_plan)
        print(plan_set_dic)
        ...
        camera_set_dic = json.loads(data_camera)
        print(camera_set_dic)
        camera_intrinsic = np.array(camera_set_dic["intrinsic"]) #行列
        tool_points2 = projection(T_Camera_Tool2_array, plan_length, camera_intrinsic)#行列,<class 'numpy.ndarray'>
        
        tool_points2_json = json.dumps({"tool_points2" : tool_points2.tolist()})
        client.send(tool_points2_json.encode())
        conn.close()
        break
