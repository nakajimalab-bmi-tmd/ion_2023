...
svr_q_dic = {"semantics" : {"entity": "PID", "baseentity" : "-", "role" : "T_Camera_Plan2", "spatial" : "any", "temporal" : "any"}, "dataproperty": {"format": "inaoka_matrix"}}
...
while True:
    ...
    while True:
        ...
        #カメラトラッカー
        T_CameraTracker2 = np.array(tracker_dic["CameraTracker"])
        #ボーントラッカー
        T_BoneTracker2 = np.array(tracker_dic["BoneTracker"])
        ...
        T_Camera_Plan2 = T_Camera_CameraTracker_array.dot(np.linalg.inv(T_CameraTracker2)).dot(T_BoneTracker2).dot(T_BoneTracker_Plan) #行列,<class 'numpy.ndarray'>

        T_Camera_Plan2_json = json.dumps({"T_Camera_Plans2" :  T_Camera_Plan2.tolist()})
        client.send(T_Camera_Plan2_json.encode())
        conn.close()
        break
