...
svr_q_dic = {"semantics" : {"entity": "PID", "baseentity" : "any", "role" : "T_BoneTracker_Tool", "spatial" : "any", "temporal" : "any"}, "dataproperty": {"format": "inaoka_matrix"}}
...
while True:
    ...
    while True:
        ...
        #plan_result = get_IRISobj_dic(cls="navi", R="surgical_result", E=cli_dic["semantics"]["entity"], BE="-", T=cli_dic["semantics"]["temporal"], S=cli_dic["semantics"]["spatial"],P=json.dumps({"format": "inaoka_json"}))
        result_json = get_data_ID(cls="ion.navi",R="surgical_result", E=cli_dic["semantics"]["entity"], BE="-", T=cli_dic["semantics"]["temporal"], S=cli_dic["semantics"]["spatial"],P=json.dumps({"format": "inaoka_json"}), start=1, end=(maxID+1))
        data_result = get_data(result_json[0]["ID"])
        result_dic = json.loads(data_result)
        plan1_num = result_dic["PlanNumber"][0]
        T_BoneTracker = np.array(result_dic["BoneTracker"])
        T_ToolTracker = np.array(result_dic["ToolTracker"])
        print(T_BoneTracker)
        print(T_ToolTracker)
        tool_length1 = [0.0, 90.0]
        deg_90 = 0.5 * np.pi
        R, J = cv2.Rodrigues(-deg_90 * np.array([1.0, 0.0, 0.0]))
        T_ToolTracker_Tool = np.eye(4)
        T_ToolTracker_Tool[0:3, 0:3] = R
        T_ToolTracker_Tool[0:3,3] = np.array([-45.8,-213.4, -3.6])
        T_BoneTracker_Tool = np.linalg.inv(T_BoneTracker).dot(T_ToolTracker).dot(T_ToolTracker_Tool)
        print(T_BoneTracker_Tool)
        T_BoneTracker_Tool_json = json.dumps({"T_BoneTracker_Tool" : T_BoneTracker_Tool.tolist()})
        client.send(T_BoneTracker_Tool_json.encode())
        conn.close()
        break
