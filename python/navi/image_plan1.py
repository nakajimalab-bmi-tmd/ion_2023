...
svr_q_dic = {"semantics" : {"entity": "PID", "baseentity" : "-", "role" : "image_plan1", "spatial" : "any", "temporal" : "any"}, "dataproperty": {"format": "inaoka_matrix"}}
...
# Create an iris object
        ...
        plan1_json = get_data_ID(cls="ion.navi", R="surgical_result",E= cli_dic["semantics"]["entity"],BE="-",T=cli_dic["semantics"]["temporal"],S=cli_dic["semantics"]["spatial"], P=json.dumps({"format": "inaoka_json"}), start=1,end=(maxID+1))
        print(plan1_json)
        ...
        #番号から対象の画像を取得
        image1 = cv2.imread(plan1_num + '.png')

        im_height, im_width, dummy = image1.shape
        image1_gray_ch1 = cv2.cvtColor(image1, cv2.COLOR_BGR2GRAY)
        image1_gray = cv2.merge([image1_gray_ch1,image1_gray_ch1,image1_gray_ch1])

        cliq2 = cli_dic.copy()
        cliq2["semantics"]["role"] = "plan_points1"
        port_data, addr_data = UDP_broadcast(json.dumps(cliq2, ensure_ascii=False))
        if json.loads(port_data)["msg"] == "ok":
            
            plan_point1_bin = TCP_data_rcv(addr_data,port_data)
            print(plan_point1_bin)
            plan_point1_ls = json.loads(plan_point1_bin)["plan_points1"]
            plan_point1_array = np.array(plan_point1_ls)
            print(plan_point1_array)
        else :
                break
        
        image_plan1 = draw_line(image1_gray,plan_point1_array, (0, 255, 0), 1.0)
        print(image_plan1.dtype)
        cv2.imwrite("1.png", image_plan1)
        image_plan1_json = json.dumps({"image_plan1" : image_plan1.tolist()})
        #print(image_plan1_json)
        client.send(image_plan1_json.encode())
        conn.close()
        break
