...
if __name__ == '__main__':
    start = time.time()
    cls_name = "ion.program"
    maxID_prg = irispy.classMethodValue(cls_name, "getmaxID")

    #プログラムを通したデータの取得
    q = {"Role" : "vital_all", "Entity" : "PID001", "BaseEntity" : None, "Temporal" : None, "Spatical" : None, "Property" : json.dumps({"format": "inaoka_program_sample2_csv"})}
    # sum_time, i = get_prgID_time(cls=cls_name, R=q["Role"], E=q["Entity"], BE=q["BaseEntity"], T=q["Temporal"], S=q["Spatical"], P=q["Property"], start=15, end=(maxID_prg+1))

    ls_prg = get_program_ID(cls=cls_name, R=q["Role"], E=q["Entity"], BE=q["BaseEntity"], T=q["Temporal"], S=q["Spatical"], P=q["Property"], start=15, end=(maxID_prg+1))
