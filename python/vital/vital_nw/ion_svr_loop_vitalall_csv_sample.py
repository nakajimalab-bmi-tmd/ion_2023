...
#main
#server側のクエリ（担当のクエリ）
svr_q_dic = {"semantics" : {"entity": "PID", "baseentity" : "any", "role" : "vital_all", "spatial" : "any", "temporal" : "any"}, "dataproperty": {"format": "inaoka_program_sample_csv"}}
...
while True:
    ...
    while True:
        ...
        filename = f_datetime+"_vital_all.csv"
        
        with open(filename, "w") as f:
            write_csv = csv.writer(f)
            write_csv.writerow(["patient_ID", "vital_name", "vital_value", "time"])
            for i in range(len(vall_prg_ls)):
                write_csv.writerow([vitall_all_q["semantics"]["entity"], vall_prg_ls[i]["Role"], vall_prg_ls[i]["DataBody"], vall_prg_ls[i]["Temporal"]])
