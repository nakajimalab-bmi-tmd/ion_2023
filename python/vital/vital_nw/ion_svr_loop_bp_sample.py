...
#main
#server側のクエリ（担当のクエリ）
svr_q_dic = {"semantics" : {"entity": "PID", "baseentity" : "any", "role" : "bloodpressure", "spatial" : "any", "temporal" : "any"}, "dataproperty": {"format": "inaoka_program_sample"}}
...
while True:
    ...
    while True:
        ...
        ls_de = get_IRISobj_ls(cls="ion.bital", R="機器ID", E="血圧計", BE=None, DB=None, T=None, S=None, start=1, end=(maxID+1))
        ...
        for i in range(len(ls_de)):
            ls_all_dia = get_IRISobj_dic(cls="ion.bital", R="diastole", E=ls_de[i][3], BE=None, DB=None, T=None, S=None, start=1, end=(maxID+1))
            ls_all_mean = get_IRISobj_dic(cls="ion.bital", R="mean", E=ls_de[i][3], BE=None, DB=None, T=None, S=None, start=1, end=(maxID+1))
            ls_all_sys = get_IRISobj_dic(cls="ion.bital", R="systole", E=ls_de[i][3], BE=None, DB=None, T=None, S=None, start=1, end=(maxID+1))
            
        ls_all_bp = ls_all_dia+ls_all_mean+ls_all_sys    
        result_ls = []
        for i in range(len(ls_de_use)):
            for j in range(len(ls_all_bp)):
                if ls_de_use[i]["Data Body"] == ls_all_bp[j]["Entity"] and (0 <= (datetime.datetime.strptime(ls_all_bp[j]["Temporal"], '%Y/%m/%d %H:%M:%S.%f')-datetime.datetime.strptime(ls_de_use[i]["Temporal"], '%Y/%m/%d %H:%M:%S.%f')).total_seconds() <= ls_de_use[i]["period"]):
                    result = {"Entity" : ls_de_use[i]["Entity"], "Role" : ls_all_bp[j]["Role"], "DataBody": ls_all_bp[j]["Data Body"],"Temporal" : ls_all_bp[j]["Temporal"] }
