...
def get_program(num):
    ...
    data = ""
    while True:
        chunk = irispy.classMethodValue("FS.if30recursive","getprogrampy", str(num), step, pos)
        data += chunk
        if len(chunk) < step:
            break
        pos += step
...
if __name__ == '__main__':
    ...
    sum_time, i = get_prgID_time(cls=cls_name, R=q["Role"], E=q["Entity"], BE=q["BaseEntity"], T=q["Temporal"], S=q["Spatical"], P=q["Property"], start=61, end=(maxID_prg+1))
    time_dic = {str(i):sum_time}
    ls_prg = get_program_ID(cls=cls_name, R=q["Role"], E=q["Entity"], BE=q["BaseEntity"], T=q["Temporal"], S=q["Spatical"], P=q["Property"], start=61, end=(maxID_prg+1))
