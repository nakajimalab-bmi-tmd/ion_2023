...
if __name__ == '__main__':
    ...
    #プログラムを通したデータの取得
        ...
        sum_time, i = get_prgID_time(cls=cls_name, R=q["Role"], E=q["Entity"], BE=q["BaseEntity"], T=q["Temporal"], S=q["Spatical"], P=q["Property"], start=78, end=(maxID_prg+1))
        time_dic = {str(i):sum_time}
        ls_prg = get_program_ID(cls=cls_name, R=q["Role"], E=q["Entity"], BE=q["BaseEntity"], T=q["Temporal"], S=q["Spatical"], P=q["Property"], start=78, end=(maxID_prg+1))

        result = get_result_from_prg(ls_prg, q)
        
        result.append(time_dic)
        for j in range(1,11):
            print(result[j][str(88-j)])
